...
class Grapher(object):
    ...
    def plotGrowth(self, state_lst):
        # fig = go.Figure()
        fig = make_subplots(
            rows = 1, cols = 2,
            subplot_titles = ("New York", "New Jersey")
        )
        ...
        for region_name, region in self.regional_data.items():
            ...
            for state, stateDF in region.groupby('state'):
                if state in state_lst:  
                    print(i, j)
                    print(state)
                    fig.add_trace(self._addState(state, stateDF, fig, region_name), row = 1, col = j)
                    j += 1




        py.iplot(fig, filename = 'states individual')
        # fig.update_layout(
...
if __name__ == '__main__':
    ...
    coronaON419 = Grapher(df)
    coronaON419.plotGrowth(["New York", "New Jersey"])
    # coronaON419.plotChoropleth()
